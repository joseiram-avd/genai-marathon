...
#Install libraries
%pip install mlflow==2.10.0 lxml==4.9.3 langchain==0.0.344 databricks-vectorsearch==0.22 cloudpickle==2.2.1 databricks-sdk cloudpickle==2.2.1 pydantic datasets transformers
...
# DBTITLE 1,Retrieve Product Data from Table
# Retrieve the data from the table tb_product_silver in the genai_bronze_dev.database
# df = spark.sql("SELECT product FROM genai_bronze_dev.database.tb_products_silver as a").filter("product is not null")
...
# DBTITLE 1,Product List Conversion
# product_list  = ["Empanado A Base De Carne De Frango Sabor Queijo Mini Chicken Perdigao Sachet 275g"
#                  ", EMPANADO MINI CHICKEN PERDIGAO 275g"]

# Get the column values as a list
product_list = df.select("product").limit(limit_records).rdd.flatMap(lambda x: x).collect()
...
# DBTITLE 1,Save Dict to Delta Table
from pyspark.sql.types import StructType, StructField, StringType

def save_dict_to_delta_tablen(data_dict, schema, table_name, recreate=False):
...
5) PRODUTO_BASE: Nome mais genérico possível, com uma ou duas palavras. CATEGORIA: Alimento ou Bebida ou Não Alimento. 
6) COMESTIVEL SIM ou NÃO.
...
# DBTITLE 1,Save LLM Chain Response to Table
import json 
...
print( llm_chain_response.strip("") )
# Convert llm chain response into dict 
data_dict = json.loads(llm_chain_response)

# Save the llm chain response into target silver table  
df = save_dict_to_delta_tablen(data_dict, schema, table_name, True)
